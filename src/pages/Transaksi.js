...
export default class Transaksi extends React.Component {
	...
	addDetail = () => {
		...
		// let initPaket = this.state.paket[0]['id']
		this.setState({
			id_paket: 0,
			qty: 0
		})
	...
	saveDetail = event => {
		...
		const form = {
			id_paket: parseInt(this.state.id_paket),
			qty: parseInt(this.state.qty)
		}

		console.log(form)

		let detailTemp = this.state.detail_transaksi
		detailTemp.push(form)

		this.setState({detail_transaksi:detailTemp})
	...
	saveTransaksi = event => {
		...
		const form = {
			...
			id_member: parseInt(this.state.id_member),
			status: this.state.status,
			id_outlet: parseInt(this.state.id_outlet),
								...
								<div className="modal-body">
									<form onSubmit={ev => this.saveTransaksi(ev)}>
										...
										<table className="table table-striped">
											...
											<tbody>
												{this.state.detail_transaksi.map((detail, i) => (
													<tr>
														<th scope="row">{i + 1}</th>
										...
										<button type="button" className="btn btn-success" onClick={() => this.addDetail()}>
											Add Detail
										</button>
